...
Proof with auto.
  ...
  - remember (sup A) as σ. 超限讨论 σ.
    + apply 仅零或一之并为零 in H1 as []. exfalso...
      apply 序数集的并为上界... rewrite H. rewrite 单集之并. apply 替代介入_自动... 
    + apply 序数集的并为上确界... apply 替代介入_自动.
      apply 如果序数集的上确界为后继_那么它在序数集内... exists σ...
    + copy HF as [_ [_ 连续]]. rewrite 连续... apply 序数集的并为上确界.
      * intros x Hx. apply 替代除去 in Hx as [α [Hα H]].
        subst x. apply HF. eauto.
      * split... intros ξ Hξ. left.
        apply 替代除去 in Hξ as [α [Hα H]]. subst ξ σ.
        apply 并集除去 in Hα as [β [Hβ Hα]].
        apply 集族并介入 with β... apply 序数嵌入保序...
  - apply 序数集的并为上确界... split... intros y Hy.
    apply 替代除去 in Hy as [α [Hα H]]. subst y...
    apply 序数集的并为上界 in HA as [_ Hle]. apply Hle in Hα as [].
    left. apply 序数嵌入保序... right. congruence.
Qed.
...
Proof with auto.
  ...
  - assert (∃α ⋵ C, α ∉ ran f). {
    ...
    + apply Hle. apply 小于等于即小于后继... split...
...
Proof with auto.
  unfold β. rewrite (上确界的嵌入等于嵌入集的上确界 F F嵌入 A A非空 A为序数集).
  外延.
...
Proof with auto.
  ...
  assert (H: F β = β). apply β为不动点...
  set (λ β, F β = β) as P.
  pose proof (满足条件的最小序数为之 β Hβ P H) as [Hβ₀ [不动 Hle]].
  split3... intros. 排中 (γ ⋸ β).
  - apply 小于等于即小于后继 in H2...
  - apply 序数可换 in H2... apply 序数传递_弱 with β...
Qed.
...
Local Notation β := (任意不动点 F α⁺).

Definition 后继不动点 := inf {γ ∊ β⁺ | F γ = γ ∧ α ∈ γ}.
