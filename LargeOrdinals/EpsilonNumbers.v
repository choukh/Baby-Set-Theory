...
Local Hint Resolve ε为序数运算 : core.

Lemma ε为序数嵌入 : 为序数嵌入 ε.
Proof. apply 不动点枚举为序数嵌入. auto. Qed.
Local Hint Resolve ε为序数嵌入 : core.
...
Proof with auto.
  intros α Hα. apply 不动点不为零... intros H. apply 幂为零 in H...
Qed.
...
Theorem 最小不动点表达为任意不动点 : ∀ ϕ ψ, 为序数嵌入 ϕ →
  ψ = E ϕ → ψ ∅ ≠ ∅ → E ψ ∅ = A ψ (ψ ∅).
Proof with auto.
  intros ϕ ψ ϕ嵌入 不动 H0.
  assert (H1: 为序数嵌入 ψ). subst. apply 不动点枚举为序数嵌入...
  assert (H2: E ψ ∅ ⋵ 𝐎𝐍). rewrite 不动点枚举_0. apply 最小不动点为之...
  assert (H3: ψ ∅ ⋵ 𝐎𝐍). subst. rewrite 不动点枚举_0. apply 最小不动点为之...
  assert (H4: A ψ (ψ ∅) ⋵ 𝐎𝐍). apply 不动点为之...
  apply 包含的反对称性.
  - apply 小于等于即包含... rewrite 不动点枚举_0.
    apply 最小不动点为之... apply 不动点为之...
  - intros x Hx. apply 集族并除去 in Hx as [n [Hn Hx]].
    apply 序数传递 with (序数递归 (ψ ∅) ψ n)... clear Hx. 归纳 n.
