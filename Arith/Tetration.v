...
Lemma 仅零之左迭代为零 : ∀ α β ⋵ 𝐎𝐍, α ^^ᴸ β = 0 → α = 0.
Proof with eauto.
...
Theorem 左迭代在ω次之后不变 : ∀α ⋵ 𝐎𝐍, 1 ∈ α → ∀β ⋵ 𝐎𝐍, ω ∈ β → α ^^ᴸ β = α ^^ᴸ ω.
Proof with auto.
...
Theorem 右迭代化简 : ∀ α β ⋵ 𝐎𝐍, α ≠ 0 → α ^^ᴿ β = α ^ (α ^ β).
Proof with auto.
  intros α Hα.
  排中 (α = 1) as [|H1]. {
...
Theorem 无限顶迭代等于左迭代 : ∀α ⋵ 𝐎𝐍, ∀τ ∈ α, 1 ∈ τ → (α ^^ᵀ ω) τ = α ^^ᴸ ω.
Proof with auto.
