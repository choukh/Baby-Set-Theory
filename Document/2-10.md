...
本文是Coq集合论讲座第2季第10讲。上一讲：[ocau：Coq集合论讲座（2-9：Veblen不动点）](https://zhuanlan.zhihu.com/p/432777891)
...
给定序数嵌入F和序数α，由“任意不动点”，我们可以构造不小于α⁺的不动点β（即α⁺ ⋸ β）。也就是说，存在大于α的不动点β（即α ∈ β），于是可以用“满足条件的最小序数”构造大于α的最小不动点β₀，它就是我们想要的后继不动点。
...
不难证明以上构造满足要求。注意在Coq证明中我们先用cut改变了一下命题的表述形式，于是可以顺利地使用“满足条件的最小序数为之“完成证明。
...
Lemma 后继不动点为之 : β₀ ⋵ ON ∧ F β₀ = β₀ ∧ α ∈ β₀ ∧
  ∀γ ⋵ ON, F γ = γ → α ∈ γ → β₀ ⋸ γ.
Proof with auto.
  cut (β₀ ⋵ ON ∧ (F β₀ = β₀ ∧ α ∈ β₀) ∧ ∀γ ⋵ ON, (F γ = γ ∧ α ∈ γ) → β₀ ⋸ γ).
...
Lemma 加法放大_右 : ∀ α β ⋵ ON, β ≠ 0 → α ∈ α + β.
Lemma 乘法放大_右 : ∀ α β ⋵ ON, α ≠ 0 → 1 ∈ β → α ∈ α * β.
```
...
Lemma 加法放大_左弱 : ∀ α β ⋵ ON, α ⋸ β + α.
Lemma 乘法放大_左弱 : ∀ α β ⋵ ON, β ≠ 0 → α ⋸ β * α.
```
